          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-5">
            <motion.div variants={fadeUp} className="lg:col-span-2">
              <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-glow">
                <p className="text-sm font-semibold text-white">Fast response. Clear next steps.</p>
                <p className="mt-2 text-sm leading-relaxed text-white/70">
                  Share a bit about your project and we’ll reply with a short plan, timeline, and next steps.
                </p>
                ...
                <div className="mt-6 space-y-4">
                  <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-white/80">hello@novasphere.studio</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                    ...
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-widest text-white/50">Location</p>
                      <p className="mt-1 text-sm text-white/80">Remote • Worldwide</p>
                    </div>
                  </div>
                </div>

                <div className="mt-6 rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                  <p className="text-xs font-semibold uppercase tracking-widest text-white/50">What you’ll get</p>
                  <ul className="mt-3 space-y-2 text-sm text-white/70">
                    {['Timeline + scope', 'Price estimate', 'Launch checklist'].map((i) => (
              ...
              <form onSubmit={onSubmit} className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-glow">
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="space-y-2">
                    <span className="text-xs font-semibold uppercase tracking-widest text-white/50">Name *</span>
                    <input
                      name="name"
                      value={form.name}
                      onChange={onChange}
                  ...
                  <label className="space-y-2">
                    <span className="text-xs font-semibold uppercase tracking-widest text-white/50">Company</span>
                    <input
                      name="company"
                      value={form.company}
                      onChange={onChange}
                  ...
                  <label className="space-y-2">
                    <span className="text-xs font-semibold uppercase tracking-widest text-white/50">Budget</span>
                    <select
                      name="budget"
                      value={form.budget}
                      onChange={onChange}
                ...
                <label className="mt-4 block space-y-2">
                  <span className="text-xs font-semibold uppercase tracking-widest text-white/50">Message *</span>
                  <textarea
                    name="message"
                    value={form.message}
                    onChange={onChange}
